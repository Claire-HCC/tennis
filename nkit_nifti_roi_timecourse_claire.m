...
function nkit_nifti_roi_timecourse_claire(datafiles_bothSchool, roifiles, opts)
...
for schi=1:2;
    sch=schools{schi};
    datafiles=datafiles_bothSchool(:,schi);
    
    mcutoff=opts.mcutoff; % mean (over time) value at a voxel must be at least this high to be retained
    ...
    if strcmp(opts.roidata,'savedata');
        ...
        %        fprintf(['Loading Pair ' num2str(iscpair(sid)) ' ' sch ' mat file\n']);
                load([datafiles{sid}(1:end-3) 'mat']);
            else % otherwise load the nifti and save a .mat version for next time
       %         fprintf(['Loading pair ' num2str(iscpair(sid)) ' ' sch ' Nifti file\n']);
    ...
    if strcmp(opts.roidata,'loaddata');
        nansubs = [];
        for sid = 1:length(datafiles);
        ...
        eval(['roitc' num2str(schi) '=roitc;']);
        eval(['meantc' num2str(schi) '=meantc;']);
        eval(['uncropped_roitc' num2str(schi) '=uncropped_roitc;']);
    end
end
...
if strcmp(opts.roidata,'loaddata');
    for sid = 1:length(datafiles);
        if ismember(sid,nansubs), continue; end
        roicorr(sid) = corr(roitc1(:,sid),roitc2(:,sid));
    end
    roicorr(nansubs) = NaN;
    ...
    save(savename,'roitc1','roitc2','meantc1','meantc2','uncropped_roitc1','uncropped_roitc2','meancorr','roicorr');
